...
CKObjectDeclaration *FillBehaviorBendDecl()
{
    CKObjectDeclaration *od = CreateCKObjectDeclaration("Bend");
...
int Bend(const CKBehaviorContext &behcontext)
{
    ...
    /*
        ...
        Vx3DMatrixFromRotation(tmAbove,vy,angle);

        VxMatrix tmBelow;
        Vx3DMatrixFromRotation(tmBelow,vy,angle);
    */
    VxBbox bbox;
    ...
    case 0:
    {
        VxVector vz(0.0f, 1.0f, 0.0f);
        Vx3DMatrixFromRotation(mat, vz, -HALFPI);
    }
    break; // x
    case 1:
    {
        VxVector vx(0.0f, 0.0f, 1.0f);
    ...
    {
    case 0:
        len = bbox.Max.x - bbox.Min.x;
        break;
    case 1:
        len = bbox.Max.y - bbox.Min.y;
        break;
    case 2:
        len = bbox.Max.z - bbox.Min.z;
        break;
    }
    ...
    if (CKIsChildClassOf(mesh, CKCID_PATCHMESH) || !normalc)
        narray = NULL;
